...
% Patrick editing this to rerun old trials.
%
clear all;
% close all;
clc;
...
crashes = load('trial_data/crash_filenames.mat');
crash_file = crashes.crash_filenames{1};
crash = load(crash_file);
...
% wipe plot data field of obstacles
for i = 1:length(crash.summary.obstacles)
    crash.summary.obstacles{i}.plot_data.body = [];
end
...
W = zonotope_box_world('verbose',verbose_level,'N_tall',N_tall,...
    ...
    'obstacles', crash.summary.obstacles) ;

...
%             Zproj = R_obs{i}{k};
            % lolz hack
            fake_c = (R_obs{i}{k}(:, 2) + R_obs{i}{k}(:, 1))/2;
            fake_G = [(R_obs{i}{k}(1, 2) - R_obs{i}{k}(1, 1))/2, 0; 0, (R_obs{i}{k}(2, 2) - R_obs{i}{k}(2, 1))/2];
...
%     randk = obs_idx;
    for i = 1:length(randk)
%         randu = randPoint(project(R_obs{I(i)}{J(i)}, slice_dim));
        fake_c = (R_obs{I(i)}{J(i)}(:, 2) + R_obs{I(i)}{J(i)}(:, 1))/2;
        fake_G = [(R_obs{I(i)}{J(i)}(1, 2) - R_obs{I(i)}{J(i)}(1, 1))/2, 0; 0, (R_obs{I(i)}{J(i)}(2, 2) - R_obs{I(i)}{J(i)}(2, 1))/2];
        Zproj = zonotope([fake_c, fake_G]);
        randu = randPoint(Zproj);
        my_vpeak_1 = randu(1);
        my_vpeak_2 = randu(2);
        [tout1, xout1] = ode45(@dyn_quadrotor_toPeak, [0:0.01:1], [0;-5;5;my_vpeak_1;0]);
        [tout1, yout1] = ode45(@dyn_quadrotor_toPeak, [0:0.01:1], [0;-5;5;my_vpeak_2;0]);

function [E_lo, E_hi] = put_error_in_time_bins(E,T,t_lo,t_hi)
    ...
    % compute E_lo
    E_lo = [min(Ex,[],2,'omitnan'), min(Ey,[],2,'omitnan'), min(Ez,[],2,'omitnan')] ;
    E_hi = [max(Ex,[],2,'omitnan'), max(Ey,[],2,'omitnan'), max(Ez,[],2,'omitnan')] ;
end